...
def validate_authenticity(orig_post: dict, violator_post: dict):
    ...
    orig_ch_name, orig_post_id = extract_channel_and_post_ids(orig_post["owner_link"])[0]
    vio_ch_name, vio_post_id = extract_channel_and_post_ids(violator_post["owner_link"])[0]

    if orig_ch_name == vio_ch_name:
        raise Exception(f"Both posts are from the same origin")
    if not orig_post["time_val"] > violator_post["time_val"]:
        raise Exception(f"Original post's time is not prior to violator's post")

    similarity = check_similarity(
        orig_post["post_content"], violator_post["post_content"]
    )
    write_values(
        similarity, threshold, violator_post["post_url"], orig_post["post_url"], violator_content=violator_post['post_content']
    )
